...
- only an MLE approach to model SVC and to give predictions. 
...
3. *mean-zero SVC*, i.e. only a zeo-mean random effect modelling the spatial structure. 
...
where $\textbf X$ is our model matrix. Then we can write down the model devided into an fixed effect part and a random effect part:


$$\textbf y(\textbf s) = \textbf X  \mu + \textbf W  \tilde \beta(\textbf s) + \epsilon( \textbf s)$$

where $\tilde{\beta}$ is a mean-zero GRF. Note that both model are the same if $\textbf X  = \textbf W$. Thus, we can specify options 1 to 3 from above by in- or excluding columns of $\textbf X$ or $\textbf W$.
...
To give a simple example, we start by sampling artifical data. So define an *full SVC model* as given above and sample from a regular grid using a help function:
...
The main function of this package is `SVC_mle`. Its function call starts the MLE but it requires some preperation and settings of control parameters. We go through each argument of the `SVC_mle` function and its control parameter `SVC_mle.control`. 
...
As for the control parameters for `SVC_mle`, we go through them as they are implemented in the current version of `varycoef`. By calling `SVC_mle.control`, we create an list with all needed arguments to start a simple `SVC_mle`. 
...
For the scale, we remind ourselves that the effective range of an exponential GRF is approximately 3 times its range. Since in our case we are in an restricted domain of the unit square, any dependency structure on a distance above the maximum distance in the domain, we cannot not model. Therefore we suggest to take an initial scale of a third or fourth the maximum distance. So in our case, we take
...
One can either overwrite the default initial value (`control$init`) or call `SVC_mle.control` with corresponding arguments:
...
In this small example we already can see that the predicted SVC takes the general spatial structure of the true SVC. The error does not appear to have spatial structure for the SVC 2 and 3, resprectively. However, the error for the intercept seems to have some spatial structure. If we increase the number of observations, the picture changes:
...
As above, one can then use the `SVC_mle`-class obejct to give estimates of the SVC. In fact, we can further specify the new data matrices `newX` and `newW` to predict `y` at these locations, if wanted. Say, we want to predict the SVC for the location $(0, 0)$
