...
As above, one can then use the `SVC_mle`-class object to give estimates of the SVC. In fact, we can further specify the new data matrices `newX` and `newW` to predict `y` at these locations, if wanted. Say, we want to predict the SVC for the location $(0, 0)$
