...
The argument `cal.res` takes logical values and decides whether the residuals of the SVC model should be computed. This takes some extra time, but can be useful since an extraction of those residuals is possible using a method or `residuals`. 
...
Additionally, one can use the `fitted` to extract the fitted values from the `SVC_mle` object for the training data set.
