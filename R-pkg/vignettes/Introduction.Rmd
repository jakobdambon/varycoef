...
First, we plot the log Cadmium measurements at their respective locations. The color ranges from blue (high Cadmium measuremnets) to red (low Cadmium measurements).
...
spplot(sp_meuse, zcol = "res_lm",
     main = "Meuse River and Residuals of Linear Model") + layer(panel.lines(meuse.riv))
```
...
# visualize linear model residuals along river
spplot(sp_svc_pred, main = "Spatial Deviations of SVCs") +
    layer(panel.lines(meuse.riv))
```
