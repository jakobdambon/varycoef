...
```{r synthetic data fitted, echo = FALSE}
kable(head(fitted(fit_svc)))
```

Therefore, the SVC mentioned above is the sum of the mean value from the method `coef()` which we name $\mu_j$ and the spatial deviations from above which we name $\eta_j(s)$, i.e., $\beta_j(s) = \mu_j + \eta_j(s)$. We visualize the coefficients at their respective locations:
...
The coordinate is $s_{`r s_id`}$. Of course, for the `r s_id`th observation location the linear model is simply: 

$$y = \beta_0 + \beta_1\cdot x + \varepsilon =  `r round(coef(fit_lm)[1], 2)` + `r  round(coef(fit_lm)[2], 2)` \cdot x + \varepsilon$$

$$y = \beta_0(s_{`r s_id`}) + \beta_1(s_{`r s_id`})\cdot x + \varepsilon = `r round(coef(fit_lm)[1], 2)` + `r  round(coef(fit_lm)[2], 2)` \cdot x + \varepsilon$$
...
```{r meuse data spatial plot, echo = FALSE, message=FALSE, warning=FALSE, eval = FALSE}
...
```{r LM spatial residuals, echo=FALSE, eval = FALSE}
...
```{r meuse data  plot SVCs, echo = FALSE, eval = FALSE}
# add mean values
